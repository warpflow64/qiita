...
> [!WARNING]
> Qiita公式で提供している Qiita CLI の bun package 名は **@qiita/qiita-cli** となります。
> その他は異なるパッケージがインストールされてしまいます。必ずご確認の上、インストールしてください。

Qiita のコンテンツを管理したいディレクトリで、以下のコマンドを実行します。

```console
bun install @qiita/qiita-cli --save-dev
```
...
```console
bun install @qiita/qiita-cli@latest
```
...
>
> 例): `$ bunx qiita new newArticle001`
...
posting_campaign_uuid: null # 紐付ける記事投稿キャンペーンのUUID（UUIDは`posting-campaigns`コマンドで確認できます）
agreed_posting_campaign_term: false # true: キャンペーンの規約に同意する（`posting_campaign_uuid`を指定する場合は`true`が必須です）
---
# new article body
```
...
```console
bunx qiita publish 記事のファイルのベース名
```
...
```console
bunx qiita publish 記事ファイルのベース名 --force
# -f は --force のエイリアスとして使用できます。
bunx qiita publish 記事ファイルのベース名 -f
```
...
`bunx qiita init`コマンドで生成される`qiita.config.json`について説明します。  
...
```console
bunx qiita login --credential ./my_conf/
bunx qiita preview --credential ./my_conf/
```
...
```console
bunx qiita login --config ./my_conf/
bunx qiita preview --config ./my_conf/
```
...
```console
bunx qiita preview --root ./my_articles/
bunx qiita publish c732657828b83976db47 --root ./my_articles/
```
...
```console
bunx qiita login --verbose
bunx qiita preview --verbose
```
